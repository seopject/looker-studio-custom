...
function drawTopNpercent(data) {
  ...
  // Save raw data.
  const rawData = data.tables.DEFAULT.rows;
  ...
  // top 20%
  const zScore = 0.84;
  const thresholdValue = avg + zScore * stdDev;
  const top20PercentValues = rawData.filter(item => item[1] >= thresholdValue);
  console.log(top20PercentValues);

  // Create the title.
  const testText = document.createElement('h1');
  testText.textContent = '상위 20% 테이블 테스트'
  ...
  top20PercentValues.forEach(function(row) {
...
dscc.subscribeToData(drawTopNpercent, { transform: dscc.tableTransform });